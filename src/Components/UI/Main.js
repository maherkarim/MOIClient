...
const Main = () => {
  ...
  const appLoaded = useStoreState((state) => state.mainStore.appLoaded);
  const auth = useStoreState((state) => state.mainStore.auth);
  const config = useStoreState((state) => state.mainStore.config);
  const setIsMobile = useStoreActions(
    (actions) => actions.mainStore.setIsMobile
  );
